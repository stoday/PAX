...
@mcp.tool()
def thsr_fare_estimator(start, end, date=None):
    """
    ...
    out_date = date if isinstance(date, str) and date.strip() else datetime.today().strftime("%Y/%m/%d")
...
def tr_fare_estimator(start, end):
    """
    start: 出發站 (string)
    end: 終點站 (string)
    以內建票價表查詢台鐵票價
    ...
    data = {
        ...
        "tip114QueryVOs[0].trnDate": "2025/12/11",
        "tip114QueryVOs[0].SpecLineExtEnum": "TIP_SPEC_LINE_OTHERS",
        "tip114QueryVOs[0].directionExtEnum": "TIP_DIR_UNUSED",
        "tip114QueryVOs[0].startStation": "0990-松山",
        "tip114QueryVOs[0].endStation": "1000-臺北",
        "tip114QueryVOs[0].trainType": "3",
